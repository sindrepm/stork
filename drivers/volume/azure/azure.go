...
func (a *azure) StartBackup(
	...
	var snapshotClient compute.SnapshotsClient
	var diskClient compute.DisksClient
	// if backuplocation has creds wrt the cluster, need to use that
	azureSession := a.getAzureClientFromBackupLocation(backup.Spec.BackupLocation, backup.Namespace)
	if azureSession == nil {
		if !a.initDone {
...
func (a *azure) StartRestore(
	...
	var diskClient compute.DisksClient
	// if backuplocation has creds wrt the cluster, need to use that
	azureSession := a.getAzureClientFromBackupLocation(restore.Spec.BackupLocation, restore.Namespace)
	if azureSession == nil {
		if !a.initDone {
			if err := a.Init(nil); err != nil {
				return nil, err
...
func (a *azure) getAzureClientFromBackupLocation(backupLocationName, ns string) *azueSession {
	var azureSessionWithCred *azueSession
	...
	if len(backupLocation.Cluster.SecretConfig) > 0 {
		config := auth.NewClientCredentialsConfig(backupLocation.Cluster.AzureClusterConfig.ClientID, backupLocation.Cluster.AzureClusterConfig.ClientSecret, backupLocation.Cluster.AzureClusterConfig.TenantID)
		...
		azureSessionWithCred.snapshotClient = compute.NewSnapshotsClient(backupLocation.Cluster.AzureClusterConfig.SubscriptionID)
		azureSessionWithCred.diskClient = compute.NewDisksClient(backupLocation.Cluster.AzureClusterConfig.SubscriptionID)
